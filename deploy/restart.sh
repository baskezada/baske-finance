...
usage() {
  ...
  echo "  $0 template-prod -env prod -s api"
  exit 1
}
...
docker compose --env-file "$ENV_FILE" -p "$PROJECT" $FILES restart $SERVICE_NAMES

echo "Restart OK: project=$PROJECT env=$ENV_KIND services=${REQ_SERVICES:-all}"
